...
define CMDS
ifeq ($(1), runserver)
	envdir envs/$(ENV) armyimp/manage.py$(1)$(PORT)
else
$(1):
	envdir envs/$(ENV) armyimp/manage.py$(1)
endif
endef
...
create-db:
	envdir envs/$(ENV) createdb -U armyimp -l en_US.utf-8 -E utf-8 -O armyimp -T template0 -e armyimp

create-db-user:
	psql -d postgres -c "CREATE USER \"armyimp\" WITH PASSWORD 'armyimp' CREATEDB;"

drop-db:
	envdir envs/$(ENV) dropdb -i -e -U armyimp armyimp

drop-db-user:
	dropuser -i -e armyimp
...
isort:
	isort --recursive setup.py armyimp/ tests/
...
startapp:
	...
	mkdir -p armyimp/apps/$$app_name; \
	envdir envs/$(ENV) python manage.py startapp $$app_name armyimp/apps/$$app_name --template armyimp/config/app_template; \
	echo "Don't forget to add 'armyimp.apps."$$app_name".apps."$$app_name_title"Config' to INSTALLED_APPS in 'armyimp.config/settings/common.py'!"
...
test-upload:
	...
	python -c "import webbrowser; webbrowser.open('https://testpypi.python.org/pypi/armyimp')"

upload:
	twine upload -s dist/*
	python -c "import webbrowser; webbrowser.open('https://pypi.python.org/pypi/armyimp')"
